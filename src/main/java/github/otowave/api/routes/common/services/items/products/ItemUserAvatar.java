...
public class ItemUserAvatar extends Item {
    @Autowired
    UsersProfileRepo usersProfileRepo;

    public ItemUserAvatar(ItemModel itemModel, DefaultImageIDs defaultImageID) {
    ...
    public Mono<Void> changeImage(int newImageID) {
        return getItemEntity()
                .flatMap(userProfileEntity -> {
                    userProfileEntity.setAvatarID(newImageID);
