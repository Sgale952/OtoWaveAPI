...
public interface MusicMetaRepo extends ReactiveCrudRepository<MusicMetaEntity, Integer> {
    @Query("SELECT * FROM music_meta_entity ORDER BY uploaded DESC LIMIT :limit OFFSET :offset")
    Flux<MusicMetaEntity> findAllWithPages(@Param("limit") int limit, @Param("offset") int offset);

    @Query("SELECT * FROM music_meta_entity WHERE EXTRACT(YEAR FROM uploaded) = EXTRACT(YEAR FROM CURRENT_DATE) AND EXTRACT(MONTH FROM uploaded) = EXTRACT(MONTH FROM CURRENT_DATE) ORDER BY listens DESC LIMIT :limit OFFSET :offset")
